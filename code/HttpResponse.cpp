...
Buffer HttpResponse::makeResponse()
{
    ...
    if(statusCode_ == 400) 
    {
        doErrorResponse(output, "can't parse the message");
        return output;
    }
    ...
    {
        statusCode_ = 404;
        doErrorResponse(output, "can't find the file");
        return output;
    }
    ...
    {
        statusCode_ = 403;
        doErrorResponse(output, "can't read the file");
        return output;
    }
...
void HttpResponse::doStaticRequest(Buffer& output, long fileSize)
{
    ...
    if(keepAlive_)
    {
        ...
        output.append("Keep-Alive: timeout=" + std::to_string(g_connect_timeout) + "\r\n");
    } 
    else 
    ...
    output.append("Server: hongxingao_server\r\n");
    output.append("\r\n");
